...
- Auth is **Firebase Authentication** (email/password) with httpOnly session cookies (`POST/DELETE /api/auth/session`). Learner profiles/enrollments live in Firestore `learners/{uid}`. Protected routes are gated in `src/proxy.ts` (cookie presence) and verified with Firebase Admin in server code.
- Local/dev requires Firebase client (`NEXT_PUBLIC_FIREBASE_*`) + Admin (`FIREBASE_SERVICE_ACCOUNT_JSON` or the three `FIREBASE_*` vars), with Email/Password enabled in the Firebase Console.
- Optional integrations degrade gracefully when their env vars are unset: Stripe checkout/portal/webhook, and SMTP email (nodemailer) simply skip/soft-fail in dev. Relevant env vars (all optional for some flows): `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `NEXT_PUBLIC_URL`, `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASS`/`SMTP_FROM_EMAIL`, `CONTACT_INBOX_EMAIL`, `CONTACT_LEADS_TOKEN`, `SMTP_TEST_TOKEN`. Contact-form leads are appended to `.data/contact-leads.ndjson` when Firebase Admin is unset (do not commit this file).
