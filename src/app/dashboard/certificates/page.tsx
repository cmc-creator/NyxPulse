    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="text-slate-400 mt-1">
          Your NyxPulse Certificates of Completion. If a course also offers an American Red
          Cross pathway, you can book a skills session separately.
        </p>
      </div>
      ...
      {completedCourses.length > 0 ? (
          ...
          <div className="space-y-8">
            {completedCourses.map((course) => (
              <CertificateCard
                ...
                course={course}
                fullName={fullName}
                certificate={certificates[course.slug]}
              />
            ))}
          </div>
        </section>
        ...
        <div className="glass-card p-10 text-center">
          ...
          <p className="text-slate-400 text-sm mb-6 max-w-sm mx-auto">
            Complete all modules in a course and claim your NyxPulse certificate to see it here.
          </p>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {inProgressCourses.map((course) => (
              <div key={course.slug} className="glass-card p-5 flex items-center gap-4">
                <span className="text-2xl">{course.icon}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-white font-semibold text-sm truncate">
                    {course.shortTitle}
                  </div>
                  <div className="text-slate-500 text-xs truncate">{course.category}</div>
                </div>
                <Link
